// Generates Open Graph images for Fenster & Türen-Welt into public/og/
const fs = require('fs');
...
const WIDTH = 1200;
const HEIGHT = 630;
const BRAND = 'Fenster & Türen-Welt';
const BRAND_SUBLINE = 'Ihr Partner für Fenster, Türen, Planung & Montage';

// Page configuration matching `content/_pages.php`
const pages = [
  { slug: 'start', title: 'Fenster & Türen-Welt', subtitle: 'Ihr Partner für Fenster, Türen, Planung & Montage in Berlin' },
  { slug: 'fenster', title: 'Fenster', subtitle: 'Kunststoff-, Aluminium- und Holzfenster vom Fachbetrieb' },
  { slug: 'tueren', title: 'Türen', subtitle: 'Haustüren und Innentüren nach Maß' },
  { slug: 'planung_montage', title: 'Planung & Montage', subtitle: 'Wir planen und montieren Ihre Fenster und Türen' },
  { slug: 'kontakt', title: 'Kontakt', subtitle: 'Telefon, E-Mail oder Kontaktformular' },
  { slug: 'agb', title: 'AGB', subtitle: 'Allgemeine Geschäftsbedingungen von Fenster & Türen-Welt' },
  { slug: 'impressum', title: 'Impressum', subtitle: 'Angaben gemäß § 5 TMG' },
  { slug: 'datenschutz', title: 'Datenschutzerklärung', subtitle: 'Informationen zum Datenschutz (DSGVO)' },
];

// Color palettes per page (keyed by slug)
const palettes = {
  start: ['#8fd3f4', '#6fb1fc', '#1e69de', '#123b7b'],
  fenster: ['#8cf0ff', '#4fd2f5', '#0a8fbf', '#005f85'],
  tueren: ['#ffe29f', '#ffb347', '#ff7e5f', '#b94a5a'],
  planung_montage: ['#a1ffce', '#64e8de', '#4a90e2', '#145388'],
  kontakt: ['#99e9ff', '#52cfff', '#118bb7', '#0a5574'],
  agb: ['#d0d7e2', '#9aa7c1', '#5b6785', '#313a4d'],
...
function makeSVG({title, subtitle, brand, brandSubline, colors, logoDataUri }) {
  ...
  lines.push(`<text x="${padLeft}" y="${HEIGHT-40}" fill="rgba(255,255,255,0.80)" class="outline" font-family="Nunito, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif" font-size="24" font-weight="600" filter="url(#dropshadow)">${escapeXml('ft-welt.de')}</text>`);
  lines.push('</svg>');
  return lines.join('\n');
}
...
(async function main(){
  ...
  for (const p of pages){
    const title = p.title || 'Fenster & Türen-Welt';
    const subtitle = p.subtitle || 'Ihr Partner für Fenster, Türen, Planung & Montage';
